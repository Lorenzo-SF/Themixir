...
- **Each colour now has a strong, distinct identity.** The previous 2.0.0
  release used hardcoded tinted backgrounds that were visually similar
  across colours (all "lights" looked like pale pink, all "darks" looked
  like very-dark-grey-with-a-hint-of-X). Themes are now generated by
  calling `alaja` on each palette's own `selection` colour to produce the
  backgrounds and harmonies — so a Red theme's `light` variant has a
  saturated salmon tint, Blue's has sky tint, Green's has mint tint, etc.
  Same story for `dark`.
- **Generator fully delegates to `alaja`.** Previously the script used
  hand-rolled HSL rotation in Python for harmonies. It now shells out to
  `alaja color <hex> --harmony {triad,analogous,complementary,
  split_complementary}` and parses the table output. Results are cached
  per `(cmd)` so the 50 themes share work.
- **`Themixir.json` slimmed to schema v3.** Only `selection` + `accent`
  per colour — variants are no longer hardcoded. 10 colours × 5 variants
  is fully derived.
- **Token palette now uses the selection, not the accent.** This widens
  the perceived "family" of each theme: the strings/functions/numbers
  feel like distant cousins of the dominant selection rather than subtle
  variants of the accent.
- **WCAG auto-fix is direction-agnostic.** The previous version tried
  only one direction (lighten or darken) which could push a colour *closer*
  to the background. Now it tries both and picks the first crossing of
  the threshold, with a pure-black-or-white fallback. All 50 themes pass
  WCAG-AA on critical tokens and WCAG-large on decorative scopes.
